...
bool pragma::asset::vbsp::BSPConverter::StartConversion()
{
	...
	for(auto &str : texStringData)
	{
		...
		ufile::remove_extension_from_filename(lstr,pragma::asset::get_supported_extensions(pragma::asset::Type::Material,pragma::asset::FormatType::All));
		...
		for(auto &pair : entBlock->keyvalues)
		{
			...
			for(auto &val : pair.second)
			{
				...
				if(val.find("\x1b") != std::string::npos)
					ustring::explode(val,"\x1b",args); // Some maps have inputs separated by "\x1b", might be version dependent?
				else
