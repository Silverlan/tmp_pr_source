...
import::mdl::Bone::Bone(uint32_t mdlVersion,uint32_t id,const VFilePtr &f)
	: m_id(id)
{
	auto offset = f->Tell();

	mdl::mstudiobone_t stdBone {};
	auto headerOffset = offsetof(mstudiobone_t,pos);
	f->Read(&stdBone,headerOffset);
	if(mdlVersion == 37)
	{
		f->Seek(f->Tell() +sizeof(float) *12);

		auto startOffset = offsetof(mstudiobone_t,poseToBone);
		headerOffset = offsetof(mstudiobone_t,contents);
		f->Read(reinterpret_cast<uint8_t*>(&stdBone) +startOffset,headerOffset -startOffset);

		stdBone.quat = f->Read<Quat>();
		stdBone.contents = f->Read<int32_t>();
		f->Seek(f->Tell() +sizeof(int32_t) *3);
	}
	else
		f->Read(reinterpret_cast<uint8_t*>(&stdBone) +headerOffset,sizeof(stdBone) -headerOffset);
	auto endOffset = f->Tell();
