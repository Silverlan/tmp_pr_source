...
std::shared_ptr<pragma::asset::WorldData> source2::convert::convert_map(Game &game,std::shared_ptr<VFilePtrInternal> &f,const std::string &mapName)
{
	...
	for(auto i=decltype(models.size()){0u};i<models.size();++i)
	{
		...
		entData->SetOrigin(t.GetOrigin());

		EulerAngles ang {t.GetRotation()};
		entData->SetKeyValue("angles",std::to_string(ang.p) +' ' +std::to_string(ang.y) +' ' +std::to_string(ang.r));
